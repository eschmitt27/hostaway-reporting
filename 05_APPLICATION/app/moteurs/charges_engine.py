"""Moteur Charges pur — périmètre facture, répartition égale, ventilations analytiques.

Extrait de `app/services/charges_impact_service.py` (Mission 8) : le module était DÉJÀ pur — zéro
dépendance sqlite3, FastAPI, pandas ou fichier (audité, confirmé lors de la lecture complète du
fichier avant extraction) — et déjà la seule source de calcul (un unique appelant en production,
`charges_preview_service.compute_guidee`, ré-audité Mission 6 bis/6 ter). Contrairement au moteur
Commission (Mission 7), il n'y avait ici aucune formule dupliquée à supprimer — la seule chose qui
manquait était l'emplacement canonique (`app/moteurs/`, comme `fifo_engine.py`, Mission 5) rendant
la pureté explicite et vérifiable par un test dédié, plutôt qu'implicite dans un fichier
`app/services/*`.

`app/services/charges_impact_service.py` redevient un simple ré-export (comme
...
`tests/test_repartition_charge_commune_facture.py`) continuent de fonctionner sans modification.

RÈGLE MÉTIER FONDAMENTALE (audit Mission 6 bis, non modifiée ici) : il n'existe PAS de groupe
permanent de logements. Le périmètre d'une charge commune vient de LA CHARGE (assimilable à une
facture) : logements sélectionnés directement à sa création, et/ou logements ACTIFS d'un
propriétaire sélectionné, résolus au MOIS DE LA CHARGE (`gestion_active_pour_mois`, réutilisant
`ref_gestion_logements_hist`) — jamais un ensemble mémorisé ailleurs, jamais tous les logements
actuels du parc. Une charge directement attribuée à UN SEUL logement n'a pas besoin d'un chemin de
code séparé : `repartir_egal(montant, [UN_SEUL_LOGEMENT])` retourne mathématiquement 100 % à ce
logement — l'« affectation directe » est un cas particulier (n=1) de la même fonction de
répartition, pas une deuxième formule.

La résolution de la RÈGLE DE RÉPARTITION versionnée (`REGLE_REPARTITION_CHARGE_COMMUNE`, par date
économique = mois de la charge, `{mois}-01`) reste hors de ce moteur — responsabilité de
`charges_preview_service.compute_guidee`, déjà câblée Mission 6 ter, fail-closed (refus V27 si
aucune version ne couvre le mois), inchangée par cette extraction.
"""
...
def repartir_egal(montant: float, logements: list[str]) -> list[dict[str, Any]]:
    ...
    Cas n=1 (« affectation directe ») : retourne mathématiquement 100 % du montant à cet
    unique logement — pas un chemin de code séparé, une conséquence directe de la formule.
    """
