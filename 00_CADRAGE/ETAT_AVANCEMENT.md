...
- **Lot 3 — SAISIE_Charges_Flux FAIT (2026-06-08)** :
  ...
  - **Moteur réel Lot3 (2026-07-13, Option A)** : les 4 requêtes M-code n'ont jamais été des Power Query **vivants** (aucun connections.xml / DataMashup / queryTables) — l'onglet POWER_QUERY_CODE est **documentaire** et rien n'alimentait le MASTER. `02_TRAVAIL/lot3_generateur_charges.py` produit désormais réellement MASTER + VUE_MENAGE depuis SAISIE_Charges_Flux (contrat 37 colonnes inchangé), idempotent, avec `mois` / `impact_resultat_reel` / `impact_resultat_comptable` / `ROW_HASH` **recalculés en Python** (jamais le cache Excel des formules). `lot6f` dérive lui aussi le mois de `date_charge`. **SAISIE = vérité métier ; MASTER = sortie calculée.** Trace : CTR-LOT3-GENERATEUR-01.
