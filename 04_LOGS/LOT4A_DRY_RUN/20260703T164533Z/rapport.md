# LOT4A dry-run — Transformateur SAISIE ReservationsHH -> MASTER de test

**Statut** : `ANALYSE_BLOQUEE_TAUX`  
**as-of (date_integration)** : `2026-07-02T00:00:00Z`  
**MASTER de test genere** : False  
**Interpreteur** : `C:\Users\Ewan\miniconda3\python.exe` (Python 3.12.9)  
**openpyxl** 3.1.5

- lignes SAISIE utiles : 1
- lignes MASTER generees : 0
- lignes VUE_ACTIVE (statut_controle=VALIDE) : 0


## Invariance des sources reelles (lecture seule)

- `SAISIE_ReservationsHorsHostaway.xlsx` : INCHANGE (sha256 c3c00e730172…)
- `MASTER_FACT_MAN_ReservationsHorsHostaway.xlsx` : INCHANGE (sha256 c0e4434c3479…)
- `REF_Setup.xlsm` : INCHANGE (sha256 6d9f21de919e…)
